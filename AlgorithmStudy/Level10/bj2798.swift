...
var min = cards[0] + cards[1] + cards[2]
var sum = 0
for n1 in 0..<cards.count-2 {
    for n2 in n1+1..<cards.count-1 {
        for n3 in n2+1..<cards.count {
            sum = cards[n1] + cards[n2] + cards[n3]
            if (abs(sum - input[1]) < abs(min - input[1])) && sum <= input[1] {
                min = sum
            }
        }
    }
